...
if [ -f "config/.env" ]; then
    echo "✅ Environment file (config/.env) found"
    source config/.env
    echo "   📁 Media path: ${MEDIA_PATH:-/share/sd/SecretFolder}"
    echo "   🔌 Host port: ${HOST_PORT:-7862}"
    echo "   👤 PUID: ${PUID:-0}, PGID: ${PGID:-0}"
    echo "   🗄️  Database: ${POSTGRES_DB:-media_scoring}"
    echo "   👤 DB User: ${POSTGRES_USER:-media_user}"
else
    echo "⚠️  No config/.env file found. Using defaults."
    echo "   💡 Consider copying config/.env.example to config/.env and customizing it"
fi
...
echo "   • Access web interface: http://localhost:${HOST_PORT:-7862}"
echo "   • Connect to database: host=localhost port=${POSTGRES_PORT:-5432} db=${POSTGRES_DB:-media_scoring}"
echo "   • View logs: docker compose logs -f"
echo "   • Stop services: docker compose down"
echo ""
echo "📖 For database connection details, see DATABASE.md"