...
for /f "usebackq delims=" %%L in (`"%VENV_PY%" tools\read_config.py --file config\config.yml --format bat`) do ( %%L )
if not "%~1"=="" set "DIR=%~1"
if not "%~2"=="" set "PORT=%~2"
if not "%~3"=="" set "HOST=%~3"
...
"%VENV_PY%" run.py --dir "%DIR%" --port %PORT% --host %HOST% --pattern "%PATTERN%" --style "%STYLE%" %THUMBNAIL_ARGS% --toggle-extensions %TOGGLE_EXTENSIONS%
popd
endlocal
