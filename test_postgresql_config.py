...
def test_mine_data_respects_env_vars():
    """Test that DataMiner uses environment variables when no CLI args provided."""
    print("🧪 Testing DataMiner with environment variables...")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result_url == test_db_url:
                print("✅ DataMiner correctly uses environment variable")
            else:
                print(f"❌ DataMiner failed: expected {test_db_url}, got {result_url}")
                return False
    finally:
        # Clean up
        os.environ.pop('DATABASE_URL', None)
